...
def find_circle(img: np.ndarray) -> Iterator[Tuple[Tuple[float, float], float]]:
    ...
    circles: np.ndarray = cv2.HoughCircles(img, cv2.HOUGH_GRADIENT, 1, 20, minRadius=80)
...
def replace_circle(src_path: str, dist_path: str, show_cropped: bool):
    ...
    # Source image.
    src_img: np.ndarray = cv2.imread(src_path)
    src_gray: np.ndarray = cv2.cvtColor(src_img, cv2.COLOR_BGR2GRAY)
    ...
    # Destination image.
    dist_img: np.ndarray = cv2.imread(dist_path)
    replaced_img: np.ndarray = dist_img.copy()
    dist_gray: np.ndarray = cv2.cvtColor(dist_img, cv2.COLOR_BGR2GRAY)
    ...
    # Show images.
    cv2.imshow(f"Source: {src_path}", src_img)
    cv2.imshow(f"Destination: {dist_path}", dist_img)
    if show_cropped:
